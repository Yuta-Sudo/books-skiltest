...
if(!empty($_POST)){
	...
	$book_sql = 'UPDATE `book_recommends` SET `bookname` = ?, `member_id` = ?, `reason` = ?, `book_pic` = ?, `modified` = NOW() WHERE `recommend_id` =? ';
	$book_data = array($bookname,$_SESSION['id'],$reason,$book_pic,$_GET['id']);
												...
												<div class="login">
													...
													<!-- 本の名前 -->
														<div class="form-group" style="padding:  0px 30px;">
															...
															<input type="text" name="bookname" class="form-control" value="<?php echo $recommend['bookname']?>">
														</div>
													<!-- 本の理由 -->
														<div class="form-group" style="padding:0px 30px;">
															<label class="control-label">おすすめする理由</label>
															<textarea rows="9" style="resize:vertical; background: rgba(0, 0, 0, 0.1);" name= "reason" class="full-width form-control"><?php echo $recommend['reason']?></textarea>
														</div>
													</div>
													<div class="col-md-6 copy animate-box" >
														<div class="form-group" style="padding:0px 30px 30px 30px;">
															...
															<p class="error">* jpg、png、gifのいずれかの拡張子を選んでください。</p>
																	<?php } ?>
															<div id="bg" style="margin-left: 0px; width: 300px; height:300px;">
																<p>変更前</p>
																<img id="wait" src="pic_reommend/<?php echo $recommend['book_pic'] ?>" width="300" height="300">
															</div>
														</div>
													</div>
												</div>
...
document.getElementById("wait").innerHTML = "変更中";
  ...
  reader.onload = function(){
    ...
    document.getElementById("bg").innerHTML = "<p>変更後</p><img src='" + dataUrl + "' width='300' height='300' >";
